...
class YAMLLinter:
    ...
    def lint_file(self, file_path: Path) -> FileReport:
        ...
        try:
            ...
            return FileReport(file_path, [LintIssue(
                line=1,
                column=1,
        ...
        for i, line in enumerate(lines):
            ...
            if "\t" in line:
                report.add_issue(LintIssue(
                    ...
                    code="YML012",
                    severity=IssueSeverity.WARNING,
                    fix=lambda line_content: line_content.replace("\t", " " * self.config["indent_size"]),
                    fixable=True,
                ))
            ...
            if ":" in line and not line.strip().endswith(":"):
                ...
                if colon_pos < len(line) - 1 and line[colon_pos + 1] != " ":
                    report.add_issue(LintIssue(
                        ...
                        code="YML021",
                        severity=IssueSeverity.STYLE,
                        fix=lambda line_content: re.sub(r"(\w):(?!\s|$)", r"\1: ", line_content),
                        fixable=True,
                    ))
    ...
    def _check_indentation(self, report: FileReport, line: str, line_num: int) -> None:
        ...
        if leading_spaces > 0 and leading_spaces % expected_indent != 0:
            report.add_issue(LintIssue(
                ...
                code="YML030",
                severity=IssueSeverity.WARNING,
                fix=lambda line_content: " " * (leading_spaces // expected_indent * expected_indent) + line_content.lstrip(),
                fixable=True,
            ))
    ...
    def _check_truthy_values(self, report: FileReport, line: str, line_num: int) -> None:
        ...
        if value.lower() in truthy_mappings:
            ...
            def create_truthy_fix(old_val: str, new_val: str):
                return lambda line_content: re.sub(rf":\s+{re.escape(old_val)}\s*$", f": {new_val}", line_content)
        ...
        for i, line in enumerate(lines):
            ...
            if ":" in line_content and line_content.endswith(":"):
                report.add_issue(LintIssue(
                    ...
                    code="YML050",
                    severity=IssueSeverity.WARNING,
                    fix=lambda line_content: re.sub(r"^(\s*\w+:)\s*$", r"\1 null", line_content),
                    fixable=True,
                ))
