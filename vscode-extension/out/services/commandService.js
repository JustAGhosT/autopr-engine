...
class CommandService {
  ...
  async runQualityCheck() {
    ...
    try {
      ...
      const result = await this.executeAutoPRCommand([
        "check",
        "--mode",
        mode,
        "--files",
    ...
    try {
      ...
      if (result.success) {
        ...
        vscode.window.showInformationMessage(
          `File split analysis complete. Would create ${result.components?.length || 0} components.`,
        );
        ...
        if (proceed === "Yes") {
          ...
          if (outputDir) {
            ...
            const splitResult = await this.executeAutoPRCommand([
              "split",
              filePath,
              "--max-lines",
              maxLines,
  ...
  async toggleTool() {
    ...
    if (selectedTool) {
      ...
      vscode.window.showInformationMessage(
        `${selectedTool} ${!currentState ? "enabled" : "disabled"}`,
      );
    }
  }
  ...
  async executeAutoPRCommand(args) {
    return new Promise((resolve, reject) => {
      ...
      const process = (0, child_process_1.spawn)(
        pythonPath,
        ["-m", "autopr.cli.main", ...args],
        {
  ...
  processQualityResults(result, operation, startTime) {
    ...
    this.dataService.updatePatternSuccessRate(
      `${operation}-success`,
      result.success,
    );
  }
